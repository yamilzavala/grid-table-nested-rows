  ...
  return (
    <>
      {expanded ? (
        ...
        <button id={id} disabled={id === '0'} onClick={(e) => {e.stopPropagation; handleShowChildren(idContainer)}} className="btn-arrow ">
          <MdOutlineKeyboardArrowUp />
        </button>
      ) : (
        // <button id={id} disabled={id === '0'} onClick={() => handleShowChildren(idContainer, id)} className="btn-arrow btn-arrow-rotated">
        <button id={id} disabled={id === '0'} onClick={(e) => {e.stopPropagation; handleShowChildren(idContainer)}} className="btn-arrow btn-arrow-rotated">
          <MdOutlineKeyboardArrowUp /> 
        </button>
      )}    
    </>
